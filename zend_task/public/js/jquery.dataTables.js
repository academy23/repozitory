...
(/** @lends <global> */function($, window, document, undefined) {
	...
	DataTable.defaults = {
		...
		"oLanguage": {
			 ...
			 *    } );
			 */
			"sInfo": "Елементы с _START_ по _END_. Всего: _TOTAL_",
			 ...
			 *    } );
			 */
			"sZeroRecords": "Поис не дал результатов :-("
		},
...
$.extend($.fn.dataTableExt.oStdClasses,{sWrapper:"dataTables_wrapper form-inline"});$.fn.dataTableExt.oApi.fnPagingInfo=function(b){return{iStart:b._iDisplayStart,iEnd:b.fnDisplayEnd(),iLength:b._iDisplayLength,iTotal:b.fnRecordsTotal(),iFilteredTotal:b.fnRecordsDisplay(),iPage:Math.ceil(b._iDisplayStart/b._iDisplayLength),iTotalPages:Math.ceil(b.fnRecordsDisplay()/b._iDisplayLength)}}; $.extend($.fn.dataTableExt.oPagination,{bootstrap:{fnInit:function(b,e,c){var d=b.oLanguage.oPaginate,a=function(a){a.preventDefault();b.oApi._fnPageChange(b,a.data.action)&&c(b)};$(e).addClass("pagination").append('<ul><li class="prev disabled"><a href="#">&lt; '+'</a></li><li class="next disabled"><a href="#">'+" &gt; </a></li></ul>");e=$("a",e);$(e[0]).bind("click.DT",{action:"previous"},a);$(e[1]).bind("click.DT",{action:"next"},a)},fnUpdate:function(b,e){var c=b.oInstance.fnPagingInfo(), d=b.aanFeatures.p,a,g,i,f,h;a=Math.floor(2.5);5>c.iTotalPages?(f=1,h=c.iTotalPages):c.iPage<=a?(f=1,h=5):c.iPage>=c.iTotalPages-a?(f=c.iTotalPages-5+1,h=c.iTotalPages):(f=c.iPage-a+1,h=f+5-1);a=0;for(iLen=d.length;a<iLen;a++){$("li:gt(0)",d[a]).filter(":not(:last)").remove();for(g=f;g<=h;g++)i=g==c.iPage+1?'class="active"':"",$("<li "+i+'><a href="#">'+g+"</a></li>").insertBefore($("li:last",d[a])[0]).bind("click",function(a){a.preventDefault();b._iDisplayStart=(parseInt($("a",this).text(),10)-1)* c.iLength;e(b)});0===c.iPage?$("li:first",d[a]).addClass("disabled"):$("li:first",d[a]).removeClass("disabled");c.iPage===c.iTotalPages-1||0===c.iTotalPages?$("li:last",d[a]).addClass("disabled"):$("li:last",d[a]).removeClass("disabled")}}}});
